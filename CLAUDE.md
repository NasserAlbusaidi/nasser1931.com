...
- **Source of truth:** `NasserAlbusaidi/project-furnace` (private) at `tools/intervalsicu/` — `pull_state.py` writes the state snapshot, `workout_bank.py` is the bank module (`WORKOUT_LIBRARY` is the merged 22-workout dict), `pick_workout.py` is the canonical engine. `src/scripts/cycling-engine.mjs` is a faithful JS port of `pick_workout.py` (same thresholds, same v1 → v2 rule order). The HTML artifact at `tools/intervalsicu/cycling_coach_artifact.html` has its own simplified mini-bank — ignore for porting; the canonical Python is the reference.
...
- **v1 caveats / deferred work:**
  - **4-day forecast calendar.** Today + next 3 days as a small calendar strip at the top of `/coach`. Each tile shows the projected pick for that day under conservative assumptions (open question: do we hold recovery state constant from today, or assume AMBER as a safe default — leaning AMBER, since RED forces recovery_45 every day and a 4-day rest stripe is useless). Macro week + lift schedule + slot are already deterministic from the date; only the recovery state and v2 signals need a default. Renders inline using the same engine — no new sync needed. Replaces the single-day "today's pick" hero with a date-driven strip; today's pick stays the centerpiece, the other 3 are smaller previews.
