...
The paper's most durable finding is structural, not physiological: **across this dataset, lagged predictors that look robust in the pooled longitudinal view routinely decay when audited with walk-forward replication.** The v1 §6 headline — morning RHR predicting long-ride speed at r = −0.35 across 54 long rides ≥50 km — survived detrending, within-route replication, and a plausible mechanism, but **failed walk-forward**: train half (n=27, Dec 2024 – Aug 2025) gave detrended r = −0.51; test half (n=27, Aug 2025 – Apr 2026) gave r = +0.00; 28-day-block bootstrap CI [−0.53, +0.08] crosses zero. The signal was an Act-II onboarding-window phenomenon and has decayed. This category — *regime-bound* — sits between validated and falsified in the §8.10 ledger and accounts for as much of the surviving ledger as the validated category does (two each), and twice the falsified count outright (one). The discipline that catches it is walk-forward over leave-one-out, specified before screening, not after.

The empirical ledger across the 4-year window: **two validated cross-domain signals** — a Bayesian-reconciled logged-intake under-count of ~555 kcal/day, robust under prior sensitivity; and REM scarcity reframed from a stage-architecture failure to a sleep-duration consequence (65.4% of REM concentrated in the last third of the night). **Two regime-bound former findings** that survived their original window's checks but failed walk-forward against the historical record: morning RHR → long-ride avg speed (the §6 case, decayed Aug 2025 onward), and HRV(t−1) → next-day calorie intake — the v1-original §8.1 headline (+24.6 kcal/ms on the YTD-2026 frame, n=107, r = +0.374, Bonferroni-passed) collapsed to +7.1 kcal/ms (r = +0.14) when extended to 1,113 daily pairs from Mar 2023, and to +0.4 kcal/ms (r = +0.01) inside the cycling era 2025 (Dec '24 – Nov 19, '25, n=349) alone; the elevated slope is bound to calendar 2026, not to phase or training stress per se — within the Iron 70.3 prep phase itself (Nov 20, 2025 → Feb 13, 2026), the Nov–Dec 2025 portion (n=42) shows slope −2.7 while the Jan–Feb 2026 portion (n=45) shows +26.7. **One falsified predictor** (protein-collapse, walk-forward AUC 0.929 → 0.677 once the test window left Ramadan). **One retracted lagged correlation** (RHR(t−1) → next-day fat intake, traced to a `df.shift(1)` operation crossing missing-day gaps; r flipped +0.508 → −0.079 on consecutive-day re-alignment). **One negative result** on day-ahead recovery prediction from prior-day metrics (all models R² < 0 at n=55).
...
- *(validated)* Two cross-domain signals survive the guardrail at this n: a Bayesian-reconciled logged-intake under-count of ~555 kcal/day (§8.2), and REM as a duration-bound rather than stage-architecture phenomenon (§8.3).
- *(regime-bound)* Two former findings — RHR-on-long-rides (§6.2) and HRV-on-next-day-calories (§8.1) — survived all six guardrail checks on their original window but failed walk-forward when held against the full historical record. The §8.1 finding is the second worked retraction in the paper after §6: pooled across Mar 2023 → Apr 2026, the slope drops from +24.6 to +7.1 kcal/ms; the elevated slope is bound to calendar 2026 specifically, not to phase or training stress (the Nov–Dec 2025 portion of the same Iron 70.3 prep phase shows slope −2.7).
...
After Ramadan 2026 the training and nutrition protocol formalized into discrete phases recorded in the phase manifest. The phases are:
...
Annotated with the formal protocol phases (2026 onward, from the phase manifest) and a derived era taxonomy for the pre-protocol period:
...
| **cycling_year_one** | **2024-12-02 → 2025-11-17** | **51** | **−0.50 kg** | **2,376** | **2,457** |
| iron_70_3_prep | 2025-11-24 → 2026-02-09 | 12 | +1.90 kg | 2,593 | 2,799 |
...
- It does not show that cycling, by itself, drove any weight loss. The pre-prep cycling era (Dec 2024 → Nov 17, 2025) ended −0.50 kg from where it began; the gains and losses inside that 51-week window net to roughly zero.
...
### 5.10 The running pillar and the March 2, 2025 knee injury

Reproduction script: `paper/scripts/section5_running_facts.py`. Input: `paper/data/running_all.csv` (353 outdoor runs Sep 9, 2022 → Apr 19, 2026; treadmill sessions excluded by activity type at the puller stage; pulled via `tools/garmin/pull_running_activities.py`).

The earlier draft of this paper treated Act II as "becoming a cyclist." That framing is incomplete and in one place actively misleading. The subject was already a runner before the cycling era opened, completed a first-ever half marathon four months *into* Act II, and stopped running altogether for five months in the middle of Act II because of a knee injury that the paper did not previously acknowledge. The running record is large enough and the structural break is clean enough that Act II is better described as three sub-eras, not one.

#### 5.10.1 The four eras

| Era | Window | Days | Outdoor runs | Distance (km) | Time (h) |
|---|---|---|---|---|---|
| Act I — pre-cycling runner-and-lifter | Sep 9, 2022 → Nov 28, 2024 | 812 | 227 | 956.6 | 111.6 |
| Act IIa — runner *and* cyclist | Dec 2, 2024 → Mar 1, 2025 | 90 | 38 | 240.2 | 26.0 |
| Act IIb — knee injury, off-running | Mar 2, 2025 → Aug 10, 2025 | 161 | 19 | 69.0 | 7.4 |
| Act IIc — triathlete | Aug 11, 2025 → Apr 19, 2026 | 252 | 69 | 370.9 | 38.5 |

The 90-day Act IIa window is the only stretch in the entire dataset where the subject was running and cycling at full volume in parallel. It produced the highest training-density block on file and ended with the half marathon and the injury, in that order.

#### 5.10.2 The half marathon (Feb 21, 2025)

The first-ever half marathon was run inside Act IIa, eight days before the injury session. Logged distance 21.28 km, finishing time 2:10:10, average heart rate 166 bpm, max 179 bpm, average pace 6:07 min/km. It was named in Garmin only as "As Seeb Running" — the dataset records race effort but not race-bib status, and there is no separate event-day metadata. For the paper this is treated as a self-administered race-pace 21.1 km benchmark, not a ranked event. Its position in the timeline matters more than the time itself: a 21 km PR followed eight days later by a knee injury, followed by five months of no running, followed by a return that produced a faster 10 km benchmark inside ten months.

#### 5.10.3 The injury arc, day by day

The knee injury is dated **March 2, 2025**. The Garmin record around that date reads:

| Date | Distance | HR | Pace | Note |
|---|---|---|---|---|
| Feb 27 | 6.0 km | 152 bpm | 5:59 | Base |
| Feb 28 | 6.6 km | 168 bpm | 5:28 | Threshold (peak fitness sample) |
| Mar 1 | 7.1 km | 142 bpm | 7:16 | Recovery |
| **Mar 2** | **10.05 km** | **150 bpm** | **6:33** | **Last "normal" run — injury session** |
| Mar 4 | 6.6 km | 155 bpm | 5:55 | Anaerobic, two days post-injury |
| Mar 8 → Mar 23 | 7 short runs, 26.5 km total | — | — | Reduced-distance, reduced-intensity probing |
| **Mar 23 → Apr 6** | — | — | — | **15-day complete break, first since Sep 2022** |
| Apr 7 | 1.6 km | 144 bpm | 7:10 | Trail return attempt |
| Apr 7 → Aug 10 | 8 runs, 18.4 km total in 125 days | — | — | Effectively dead (one Doha run May 19, four As Seeb shorts Jun 18 → Jul 31) |
| Aug 11 | 3.5 km | 163 bpm | 6:37 | First proper return run |

That arc — peak → rupture → 162 days to a meaningful return run — is the cleanest within-subject structural break in the dataset. The Mar 4 anaerobic session two days after the injury reads, in retrospect, like an attempt to confirm whether the knee was a one-day problem (it was not).

#### 5.10.4 The reallocation effect on the §5 cycling story

Earlier sections (5.4 and 5.6) reported a +5.2 kph average ride speed gain at constant heart rate across five quarters and a 217 W normalized-power FTP test by November 2025, framed as "becoming a cyclist." Both are real. The framing is incomplete: a non-trivial fraction of the cycling block in Act IIb was forced reallocation from injured running, not training prioritization.

The reallocation is visible on the monthly volume table:

| Month | Outdoor runs (count / km) | Outdoor rides (count / km) |
|---|---|---|
| Jan 2025 | 10 / 55.4 | 7 / 461.6 |
| Feb 2025 | 15 / 112.8 | 3 / 119.6 |
| **Mar 2025** | **11 / 53.3** | **15 / 450.0** |
| **Apr 2025** | **1 / 1.6** | **18 / 726.2** |
| May 2025 | 1 / 2.6 | 10 / 435.7 |
| Jun 2025 | 4 / 10.2 | 11 / 386.8 |
| Jul 2025 | 2 / 5.3 | 6 / 239.2 |
| Aug 2025 | 2 / 6.6 | 10 / 503.6 |
| Sep 2025 | 7 / 30.0 | 14 / 800.6 |

In February 2025 the run/ride volume ratio was 0.94 (running and cycling roughly tied). In April 2025 it was 0.002 (running was zero, cycling at maximum). The Q3 2025 cycling quarter that the §5 story reports as "highest-volume on file" — 1,542 km / 29 rides — sits inside the running gap. None of this invalidates the cycling adaptation: the +5.2 kph at fixed HR is in the data, and structured TrainerRoad work that began in Q2 2025 is what drove the largest single-quarter speed jump. But the paper's earlier sentence "the cycling era cost 47 percent of weekly strength tonnage" needs a companion sentence: **the cycling era's middle was substantially fueled by the absence of running, not by an active choice to deprioritize it.**

#### 5.10.5 Pace at constant HR — the running-side equivalent of §5.4

The cycling section reports +5.2 kph at fixed HR across five quarters. The equivalent on the running side, filtered to road runs ≥ 3 km with average HR in the 140–165 bpm corridor:

| Quarter | n | Pace (min/km) | HR (bpm) | Avg dist (km) |
|---|---|---|---|---|
| 2024 Q4 | 28 | 6:53 | 151 | 5.6 |
| 2025 Q1 | 21 | 6:16 | 154 | 7.0 |
| 2025 Q2 | 2 | 6:47 | 150 | 3.1 |
| 2025 Q3 | 8 | 6:47 | 153 | 4.4 |
| 2025 Q4 | 23 | 6:18 | 152 | 6.6 |
| 2026 Q1 | 19 | 6:02 | 153 | 6.8 |

Pre-injury Q1 2025 sat at 6:16/km at HR 154 — a 37 sec/km improvement over Q4 2024 in twelve weeks. The Q2/Q3 2025 cells are descriptively unusable (n=2 and n=8 in the corridor) because most running stopped. Q4 2025 returned to 6:18/km at 152 bpm, essentially recovering the pre-injury pace at slightly lower HR. Q1 2026 hit 6:02/km at 153 bpm, a fresh fitness peak. The Aquathlon 10 km on December 20, 2025 (5:13/km at HR 172 — race effort, not corridor-comparable) is the cleanest single benchmark for post-injury running fitness.

#### 5.10.6 Race ladder, running-relevant

The triathlon races already covered in §5.8 are also running events; their run legs are visible via Garmin multisport splits (lap 5). One additional pure-run event sits in the record:

| Date | Event | Run leg |
|---|---|---|
| Feb 21, 2025 | First half marathon (training-named) | 21.28 km / 2:10:10 / HR 166 / 6:07 pace |
| Nov 15, 2025 | Al Bustan Sprint | 5 km (multisport lap 5) |
| Dec 20, 2025 | Aquathlon (1 km swim + 10 km run) | 10.01 km / 0:52:14 / HR 172 / 5:13 pace |
| Jan 31, 2026 | Athiba Olympic | 10 km (multisport lap 5) |
| Feb 14, 2026 | Muscat 70.3 | 21.1 km (multisport lap 5) |

The Aquathlon is the cleanest post-injury running benchmark: a standalone run race held nine months after the structural break, run at 5:13/km at HR 172, faster pace than the pre-injury half marathon at higher heart rate. The pre-injury half marathon at 21.28 km vs the Aquathlon at 10.01 km are not equal-distance comparisons, but the Aquathlon pace lies under the §5.8 race-day threshold heart-rate corridor — running fitness, by every metric available in the watch, recovered fully and then exceeded the pre-injury baseline.

#### 5.10.7 What this section does not claim

It does not claim the injury caused the cycling adaptation. The cycling adaptation has its own internal evidence (the speed-at-HR gain began in Q1 2025, *before* the injury, and the FTP curve is independent of the running calendar). What changed in the §5 narrative is the implied counterfactual: had the knee held, the §5 cycling progression would still likely have happened, but the volume that fed it during Mar–Aug 2025 would have been split with running. The single-pillar framing was the artifact, not the underlying physiology.

It also does not diagnose the injury. The Garmin record carries only date, distance, heart rate, and the subject's labeling. Mechanism, ICD code, and treatment are not in the dataset. The objective in this paper is to mark the structural break in the training record and prevent §5 from quietly assuming it didn't exist.
...
‡ The Iron 70.3 prep phase actually started Nov 20, 2025 (boundary corrected 2026-04-26 in the phase manifest), but the §7 phase-response analysis is computed from `metrics.csv` which only extends back to Jan 1, 2026. The 44-day window above is the in-2026 portion of the prep phase. The full-phase HRV→calories analysis (§8.1) pulls from MacroFactor + Garmin directly and spans the full Nov 2025 – Feb 2026 window; see the per-era decomposition in §8.1.
...
Reproduction script: `paper/scripts/section7_facts.py`. Data inputs: `data/metrics.csv` (114 rows, Jan 1 – Apr 24 2026), the phase manifest.
...
### 8.1 REGIME-BOUND: HRV and sleep → next-day calorie intake (downgraded 2026-04-26)

This subsection is the paper's second worked retraction after §6.2. The original VALIDATED finding from the YTD-2026 frame survives within that window but does not generalize backward across the 4-year record. It is regime-bound to 2026's highest-stress phases.

**Original finding (YTD-2026 frame, n = 107).** The 114-day YTD frame (Jan 1 – Apr 24, 2026) was screened with 18 Bonferroni-corrected Pearson tests at threshold p < 0.0028, plus Spearman robustness, 10,000-iteration permutation p-values, moving-block bootstrap 95% CIs (block size 5), and partial r controlling for phase and weekday. Two outcomes survived on this window:
...
Block-bootstrap 95% CI for HRV(t−1) → calories(t) on the YTD frame is [+0.27, +0.48], excluding zero. The within-window partial r controlling phase and weekday was essentially identical to the marginal r — at the time, this was read as "not a phase-confound." Practical YTD slopes on consecutive-day pairs:
...
**Held-out replication on the historical record (the retraction).** On 2026-04-26, daily Garmin HRV was bulk-pulled back to its earliest reliable date (Mar 1, 2023) and joined to the daily MacroFactor calorie record from the same window, producing 1,113 consecutive-day pairs spanning Mar 2023 → Apr 2026. The YTD-2026 frame (n = 107) is a 9.6% slice of that table. The same regression, run on each era:

| Window | n | slope (kcal per ms HRV) | r |
|---|---:|---:|---:|
| **2026-only (the original §8.1 frame)** | 108 | **+27.0** | **+0.41** |
| Cycling era 2025 (Dec '24 – Dec '25) | 391 | +0.6 | +0.01 |
| Pre-cycling (Mar '23 – Nov '24) | 614 | +5.3 | +0.11 |
| **Pooled, all 1,113 pairs** | **1,113** | **+7.1** | **+0.14** |

The calendar-2026 slope sits at the 95th-percentile end of an era distribution whose center is essentially zero. Inside the era decomposition (with the Iron 70.3 prep phase boundary corrected to its actual start of Nov 20, 2025 on 2026-04-26):

| Phase / era | n | slope (kcal/ms) | r |
|---|---:|---:|---:|
| Iron 70.3 prep, full (Nov 20 '25 – Feb 13 '26) | 87 | +9.2 | +0.13 |
| ↳ Nov 20 – Dec 31, 2025 portion | 42 | **−2.7** | −0.04 |
| ↳ Jan 1 – Feb 13, 2026 portion | 45 | **+26.7** | +0.31 |
| Ramadan cut | 29 | +11.5 | +0.21 |
| Eid Challenge | 18 | +9.7 | +0.18 |
| Home Reset | 13 | +4.5 | +0.16 |

The within-prep year split is the cleanest piece of evidence in the table: same training phase, same race-build stress, but the slope changes sign across the calendar boundary. The binding regime is calendar 2026, not the phase or its training intensity. This is the same failure mode as §6.2 (RHR → long-ride speed): the signal survives all six checks on the screening window — including phase-stratified replication — but fails Check 3 (walk-forward replication) when the held-out frame is the full pre-2026 record. The within-2026 partial-r control for phase that earlier read as "robust to confound" is now better described as: within-2026 the signal is real, but the binding regime *is* 2026 itself, and the partial-r control could not see that because no out-of-2026 data was in the regression.

**Downgrading rule.** Per §3.5 the canonical retraction trigger is "fails walk-forward replication." The §6.2 case used a 50/50 chronological split inside the cycling era; the §8.1 case uses the full 3-year held-out historical record, which is a stricter version of the same test. The pre-specified weekly walk-forward monitor (added 2026-04-26 in §9.6 H1) flagged on its first run at test/baseline ratio 0.35× — directionally consistent with the historical finding, but the historical extension supersedes it. Status: **REGIME-BOUND**.

**What is still operationally true on the YTD-2026 frame.** Inside the calendar-2026 portion of the active phase ladder (Jan–Feb 2026 prep, Ramadan, Home Reset, Eid Challenge), HRV(t−1) does carry signal about next-day intake: within-phase slopes are positive in every case, and the +24.6 kcal/ms YTD pooled slope is a reasonable summary of those four within-2026 windows. The †*regime* it lives in is best described as calendar 2026 itself, on this body, on this measurement stack — not as a stress-phase property in general (the late-2025 portion of the same Iron 70.3 prep phase shows slope −2.7 at the same training intensity). It is not a personal physiology constant; it should not be carried forward as if it were.

**Operational toolchain consequence.** `scripts/briefing.py` operationalizes the YTD slopes as a morning fueling forecast (+24.6 kcal/ms HRV, +130 kcal/hr sleep). Inside the current Eid Challenge phase that forecast is still calibrated; outside high-stress phases it is over-reactive. The briefing script is unchanged for the active phase but should be regenerated against a phase-current frame whenever the protocol exits a stress phase.

The pooled scatter, 2026-only inset, and per-era slope decomposition appear as **Figure 5** (`paper/figures/fig05_hrv_vs_next_day_calories.png`).
...
![Figure 5. HRV(t−1) → calories(t), Mar 2023 → Apr 2026 (n = 1,113 daily pairs). Panel A: pooled scatter, era-coloured; pooled OLS slope (black) is +7.1 kcal/ms with r = +0.14 — essentially flat. The calendar-2026 OLS slope (red dashed) of +27 kcal/ms is overlaid for comparison. Panel B: calendar 2026 inset (Jan 1 – Apr 24, 2026), the cluster the original §8.1 claim was made on at n = 107 / +24.6 kcal/ms / r = +0.374; the panel header reports the same calendar cluster as currently constituted (n = 108, slope +27.0 kcal/ms, r = +0.41) — one extra day appended after the Apr 24 freeze, the §8.1 prose retains the original numbers as the operative ones. Panel C: per-era OLS slope bars after the Apr 26, 2026 prep-phase boundary correction (Iron 70.3 prep widened from Jan 1, 2026 → Nov 20, 2025 start). Pre-cycling (n = 614) +5.3, Cycling 2025 (n = 349, Dec '24 – Nov 19 '25) +0.4, Iron 70.3 prep pooled (n = 87, Nov '25 – Feb '26) +9.2, Ramadan (n = 29) +11.5, Eid Challenge (n = 18) +9.7. The Iron 70.3 prep slope splits cleanly by calendar year — Nov 20 – Dec 31, 2025 portion (n = 42) is −2.7, the Jan 1 – Feb 13, 2026 portion (n = 45) is +26.7 — same training phase, opposite signs across the year boundary. The "personal physiology constant" framing collapses on contact with the historical record; the binding regime is calendar 2026, not the phase or its training intensity.](/paper/figures/fig05_hrv_vs_next_day_calories.png)

Reproduction artifacts: `paper/data/cache/hrv_daily_alltime.json` (1,150 days of Garmin HRV, Feb 2023 → Apr 2026), `paper/data/cache/daily_calories_alltime.json` (1,268 days of MF calories, Oct 2022 → Apr 2026), `paper/data/hrv_calories_daily_alltime.json` (1,113 era-tagged consecutive-day pairs), `tools/garmin/pull_hrv_bulk.py` (idempotent direct-API pull), `paper/scripts/section7_v2_facts.py` (per-era stats source).
...
The ledger uses seven status categories. **VALIDATED**, **FALSIFIED**, **RETRACTED**, and **NEGATIVE** are self-explanatory. **EMERGING** and **PARTIAL** mark findings below the §3.5 sample-size floor. **REGIME-BOUND** is the category §6.7 made necessary: a signal that survives detrend, within-phase replication, mechanism, and effect-size reporting but *fails walk-forward* — real inside one window, gone in the next. It is neither validated (does not generalize forward) nor falsified (was real when measured). The §6.2 RHR-on-long-rides finding is the canonical example, and the structural thesis of this paper (§1.4, abstract) is that this category is, in small-N longitudinal data, at least as well-represented as either of the validated/falsified poles — in this ledger it ties validated (two each) and exceeds falsified outright (one).

| § | Signal | Status | Headline number |
|---|---|---|---|
| 8.1 | HRV(t−1) / sleep(t−1) → calories(t) | **REGIME-BOUND** (downgraded 2026-04-26 from VALIDATED) | YTD-2026 calendar: r = +0.374, +24.6 kcal/ms (n=107). Pooled across Mar 2023 – Apr 2026: +7.1 kcal/ms, r = +0.14 (n=1,113). Cycling-era 2025 in isolation (Dec '24 – Nov 19 '25): +0.4 kcal/ms, r = +0.01 (n=349). Within Iron 70.3 prep itself (Nov 20 '25 – Feb 13 '26), slope flips sign across the year boundary: Nov–Dec 2025 portion −2.7 (n=42), Jan–Feb 2026 portion +26.7 (n=45). Binding regime is calendar 2026, not the phase. Same failure mode as §6.2. |
...
Two validated, two regime-bound, one falsified, one retracted, one negative, two below the sample floor, one analyst-incident — out of an order-of-magnitude-larger screen of candidate signals. Four of the five falsification/retraction/regime-bound events traced to the same root cause: the analyst (me, in an earlier session) conflated within-window variance with the population variance the test required. The remaining one (8.5) was a missing-day-shift bug in the alignment code. None of the failures were exotic. The guardrail was specified to catch exactly the failures it caught — and the two regime-bound entries (§6.2 and §8.1) both surfaced only because walk-forward replication was added after the original within-window finding was already written up. The §8.1 retraction is the second-order lesson the §6.2 retraction taught: *every* signal that survives a within-window screen earns a held-out historical replication before it becomes a claim, not after.
...
- *HRV (yesterday).* Predicts next-day calorie intake at +24.6 kcal/ms and next-day carbs at +4.25 g/ms *inside the YTD-2026 calendar frame* (§8.1) — but the §8.1 finding is now regime-bound to calendar 2026 specifically and does not generalize to the 4-year record (pooled +7.1 kcal/ms, r = +0.14, n = 1,113). The cleanest evidence: within the Iron 70.3 prep phase itself, the Nov–Dec 2025 portion shows slope −2.7 / r = −0.04 (n = 42) at the same training stress as the Jan–Feb 2026 portion's +26.7 / r = +0.31 (n = 45). Treat the +24.6 number as conditional on the YTD-2026 calendar window only; outside that, the fueling-prior interpretation is over-reactive. The detrended weekly HRV trend on long rides also has the only §6 predictor whose 95% CI excludes zero (r = +0.39, [+0.14, +0.60]; §6.3).
...
It does not buy *day-ahead prediction* on these metrics at this n. §8.6's negative result is unambiguous: at one season's worth of daily-aggregate data, day-to-day variance dominates day-to-day signal, and even the naive "today equals yesterday" predictor is worse than predicting the dataset mean. The original §8.1 finding (HRV → next-day calories at +24.6 kcal/ms) described a relationship across 107 paired days inside a single stress-phase-heavy year; held against the 1,113-pair historical record it collapses to +7.1 kcal/ms with r = +0.14 and is now regime-bound rather than validated. Even the within-window version did not predict any single day's intake to within useful precision; the held-out version tells the broader story that whatever HRV-driven appetite chain exists in this body is conditional on the regime, not a general property.
...
1. *Phase-stratified replication.* §3.5's rule that any signal existing only inside one phase is a phase note, not a general finding. The §8.4 protein-collapse predictor died on this check — 7 of 9 events were Ramadan-locked. The §8.1 HRV → calories signal *passed* this check inside the YTD-2026 frame (within-2026 partial r controlling for phase = +0.378, essentially identical to the marginal +0.374), but later failed Check 3 against the held-out historical record because the binding regime turned out to be 2026 itself — a coarser confound than within-2026 partial-r could see. Phase-stratified replication is necessary but not sufficient when the screening window is itself a regime.
...
1. **H1 — HRV → calories durability. (RESOLVED 2026-04-26.)** Originally stated as: "the §8.1 signal will hold at test/baseline slope ratio ≥ 0.5 in at least 4 of the next 8 weekly walk-forward audits." Outcome: superseded before the weekly audit accumulated by the historical extension run on 2026-04-26 (1,113 daily pairs, Mar 2023 → Apr 2026). Pooled slope collapsed from +24.6 to +7.1 kcal/ms; cycling-era 2025 slope is +0.4 kcal/ms (n=349, after the Apr 26 prep-boundary correction widened Iron 70.3 prep to a Nov 20, 2025 start); the variance is concentrated in 2026's high-stress phases. §8.1 has been downgraded from VALIDATED to REGIME-BOUND. The hypothesis is closed; the rewritten §8.1 carries the new framing.
...
| phase | string | — | must match an `id` in the phase manifest (or empty) |
...
**`paper/data/running_all.csv` (353 outdoor runs, Sep 9, 2022 → Apr 19, 2026; the §5.10 input):** `date`, `type` (running / trail_running; treadmill_running excluded at the puller stage), `name`, `distance_km`, `duration_min`, `calories`, `avg_hr`, `max_hr`, `pace_min_per_km`. Reproduction: `tools/garmin/pull_running_activities.py` (puller) and `paper/scripts/section5_running_facts.py` (analysis).
...
These scripts read from `pipeline/` only; none contain inline data-loading code. `briefing.py` is where the §8.1 HRV → next-day-calories slopes are operationalized as a daily fueling forecast. As of 2026-04-26 the §8.1 finding is regime-bound to 2026's high-stress phases (see §8.1 retraction); the briefing forecast remains calibrated for the active Eid Challenge phase but should be regenerated against a phase-current frame once the protocol exits a stress phase.
...
MacroFactor ── paper/scripts/build_weekly_longitudinal.py ── paper/data/weekly_*.csv ─── paper/scripts/section_N_facts.py ─── the data-doc (§§4–8)
```

The boundary between operational tooling and paper artifacts is deliberate. `scripts/` writes only to `data/` and `docs/`. `paper/scripts/` writes only to `paper/data/` and produces stdout numbers consumed by the data-doc. This paper does not depend on operational state — re-running every `section_N_facts.py` script against the published `paper/data/*.csv` files reproduces every number reported in §§4–8.
...
2. **§8.1 HRV → calories: RESOLVED 2026-04-26.** The first weekly walk-forward flag (test/baseline 0.35×) was superseded by a same-day historical extension run that pulled 3+ years of daily HRV from Garmin (Mar 2023 → Apr 2026) and produced 1,113 consecutive-day pairs. Pooled slope collapsed from +24.6 to +7.1 kcal/ms; cycling-era 2025 slope is +0.4 kcal/ms (n=349, after the Apr 26 prep-boundary correction widened Iron 70.3 prep to a Nov 20, 2025 start). §8.1 is now REGIME-BOUND, not VALIDATED. Reproduction artifacts: `tools/garmin/pull_hrv_bulk.py`, `paper/data/hrv_calories_daily_alltime.json`, `paper/scripts/section7_v2_facts.py`. The weekly monitor (`field_report_weekly_check.py`) is retained as a regime-stability check inside the active phase, not as a downgrade trigger — the downgrade has happened.
